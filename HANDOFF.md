...
## Состояние публикации на 31.08.2026 (вечер)

Все 11 пунктов «Контент приложения» закрыты — Console убрал чек-лист с панели управления.
В Production создан **черновик релиза с versionCode 8** (`promote --version-code 8 --draft --commit`).

**Осталось ровно два действия, оба в Console:**

1. **Тестирование и выпуск → Рабочая версия → вкладка «Страны и регионы»** — сейчас там
   **0 стран**, поэтому релиз не раскатывается и кнопка «Отправить приложение на проверку»
   на «Обзоре публикации» остаётся серой. Добавить страны (обычно — все).
2. Там же **вкладка «Выпуски» → открыть черновик 1.0.4 → «Начать развертывание в Production»**.
   Это и есть отправка на ревью: вместе с релизом уйдут витрина, рейтинг, политика и реклама.

Почему API не довёл до конца: пока приложение не опубликовано ни разу, оно числится черновиком,
и Play принимает для него только релизы со статусом `draft` («Only releases with status draft may
be created on draft app»). Раскатку первого публичного релиза Google оставляет человеку.
