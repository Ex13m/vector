...
## 2.1 Что придётся пересмотреть при выпуске v1.1 (paywall)

Анкета возрастного рейтинга (IARC) заполнена 31.08.2026 исходя из того, что приложение
полностью бесплатное. Один ответ станет недостоверным, как только появится покупка:

| Вопрос анкеты | Сейчас | После выпуска paywall |
|---|---|---|
| Могут ли пользователи покупать в приложении цифровые товары? | Нет | **Да** |

Заодно проверить: в разделе «Монетизация» появится товар (one-time, $4.99), а в витрине —
пометка «Покупки в приложении». Не забыть, иначе анкета противоречит фактам, а это нарушение
правил Play. Остальные ответы (нет общения между пользователями, местоположение другим
пользователям не передаётся, нет контента с ограничениями) при paywall не меняются.
...
**Видео-демо — ГОТОВО, ссылка для формы:**
```
https://www.youtube.com/watch?v=cgbiE_2xiUE
```
Снято 31.08.2026, 19 секунд, доступ по ссылке, канал Vector Beacon. Один непрерывный дубль
с вшитыми английскими подписями: поездка идёт и приложение на переднем плане → экран гаснет
и телефон убирается → на выключенном/залоченном экране голос продолжает объявлять цель.
В описании ролика — подробное обоснование для ревьюера (что показано по секундам, почему нужна
геолокация, что она только во время поездки, что `ACCESS_BACKGROUND_LOCATION` не объявлен).

Исходник трёхминутной съёмки: `Downloads/vector-review-demo.mp4`; нарезка — `scratchpad/cutdemo.sh`.

**Известный пробел:** в съёмке ни разу не открыта шторка уведомлений, то есть само уведомление
foreground-сервиса крупным планом не показано. Если ревьюер попросит — доснять 5 секунд:
во время активной поездки потянуть шторку вниз, показать уведомление Vector, затем «Завершить»
и что уведомление пропало.
